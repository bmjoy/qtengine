    ...
    */
    ...
    float3 calculateNonImportantLighting(float attenuation, float attenuationDotNL, float3 albedo, float3 lightColor, half dotNL)
    {
        ...
        {
            return lightColor * attenuationDotNL * albedo;
        }
        else
        {
            return lerp(lightColor * attenuation * albedo, lightColor * _LightingAdditivePassthrough * albedo * attenuation, smoothstep(_LightingAdditiveGradientStart, _LightingAdditiveGradientEnd, dotNL));
        }
    }
            ...
            calculateBasePassLighting();
                ...
                for (int index = 0; index < 4; index ++)
                {
                    ...
                    else
                    {
                        ...
                        {
                            return lerp(poiLight.color * poiLight.attenuation, poiLight.color * _LightingAdditivePassthrough * poiLight.attenuation, smoothstep(_LightingAdditiveGradientStart, _LightingAdditiveGradientEnd, 1 - (.5 * poiLight.nDotL + .5)));
                        }
                    }
                #else
